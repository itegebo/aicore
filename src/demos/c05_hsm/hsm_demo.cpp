...
/**
 * This is a custom condition implementation that checks what the last
 * requested transition was.asks the user for the result of the
 * decision.
 */
class UserCheckCondition : public aicore::Condition
{
public:

    /** Holds the option for this condition */
    unsigned myOption;
...
bool UserCheckCondition::test()
{
    printf("Checking for transition %d - ", myOption);
    if (myOption == option) {
        printf("Triggers\n");
        return true;
    } else {
...
int main(int argc, char** argv)
{
    ...
    for (unsigned i = 0; i < 15; i++)
    {
        ...
        UserCheckCondition *condition = new UserCheckCondition;
        condition->myOption = i+1;
    ...
    while (true)
    {
        ...
        // Ask for input
        printf("Which transition should be allowed to trigger (0-15)\n");
